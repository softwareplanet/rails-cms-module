#module Devcms
  class Devcms::ApplicationController < ApplicationController
    ...
    def setup_application_data
      ...
      @application_data[:lang] = Devcms::SiteLanguage.find_by_url(session[:locale]).name
    ...
    def set_locale
      ...
      if request_locale.blank? && USE_COOKIES
        request_locale = Devcms::SiteLanguage.find_by_url(cookies[:locale]).url unless cookies[:locale].blank?
      end
      # get first locale from database
      if request_locale.blank?
        request_locale = Devcms::SiteLanguage.first.url
      end
      # search for requested locale in database
      site_locale = Devcms::SiteLanguage.find_by_url(request_locale.downcase) || SiteLanguage.first
