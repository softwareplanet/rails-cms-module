...
module ActiveFile
  ...
  module Adapter
    ...
    def get_source_path
      ...
      source_path = get_source_folder + get_filename
      return source_path if File.exists?(source_path)
      # Maybe, dependency-named?
  ...
  public
    ...
    def get_attaches_by_class_name(attach_class_name)
      attach_name = TARGET_DIVIDER + type.to_s.downcase + TARGET_DIVIDER + name
      attach_folder = get_source_folder
      return Dir.glob("*#{attach_folder+attach_name}")
    end
    def set_attach(attach)
      attach_name = attach.name + TARGET_DIVIDER + self.type.downcase + TARGET_DIVIDER + name
      attach_folder = attach.get_source_folder
      ::File.open(attach_folder + attach_name, "w") do
        |file| file.write(attach.data.force_encoding('utf-8'))
      end
    end
